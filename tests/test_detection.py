...
class DetectionTests(unittest.TestCase):
    def test_ssh_brute_force_detection(self):
        events = [
            event(i, status='failure', src_ip='203.0.113.10', user='root')
            for i in range(5)
        ]
        alerts = run_detections(events)
        self.assertTrue(
            any(a.rule_id == 'DET-SSH-001' and a.severity == 'high' for a in alerts)
        )

    def test_successful_login_after_failures(self):
        events = [
            event(i, status='failure', src_ip='203.0.113.10', user='oracle')
            for i in range(4)
        ]
    ...
    def test_password_spraying_across_users(self):
        ...
        alerts = run_detections(events)
        self.assertTrue(
            any(a.rule_id == 'DET-SSH-003' and a.severity == 'high' for a in alerts)
        )
    ...
    def test_encoded_powershell_critical(self):
        ...
        self.assertTrue(
            any(a.rule_id == 'DET-PS-001' and a.severity == 'critical' for a in alerts)
        )
        ...
        alerts = run_detections(
            [
                event(
                    ...
                    message='GET /search?q=1 UNION SELECT password FROM users user_agent=sqlmap',
                )
            ]
        )
        self.assertTrue(
            any(a.rule_id == 'DET-WAF-001' and a.severity == 'high' for a in alerts)
        )

    def test_normal_web_path_does_not_match_traversal_indicator(self):
        alerts = run_detections(
            [
                event(
                    1,
                    ...
                    event_type='http_request',
                    src_ip='198.51.100.25',
                    message='GET /assets/app.js user_agent=browser',
                )
            ]
        )
        self.assertFalse(any(alert.rule_id == 'DET-WAF-001' for alert in alerts))

    def test_benign_events_no_high_or_critical_alerts(self):
        events = [
            event(i, status='success', src_ip=f'10.0.0.{i}', user=f'user{i}')
            for i in range(1, 4)
        ]
        alerts = run_detections(events)
