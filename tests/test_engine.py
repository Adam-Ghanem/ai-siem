...
from backend.main import EVENTS, app
...
class ParserTests(unittest.TestCase):
    def test_linux_auth_normalization(self):
        e = parse_event(
            'Jun 11 10:00:00 host1 sshd[1]: Failed password for invalid user root from 8.8.8.8 port 55 ssh2'
        )
        self.assertEqual(e.source, 'linux_auth')
        self.assertEqual(e.event_type, 'ssh_login')
        self.assertEqual(e.user, 'root')
        self.assertEqual(e.src_ip, '8.8.8.8')
    ...
    def test_json_event_normalization(self):
        ...
        self.assertEqual(e.source, 'edr')
        self.assertEqual(e.asset, 'win01')
...
class DetectionTests(unittest.TestCase):
    ...
    def test_success_after_failures(self):
        logs = [
            f'Jun 11 10:00:0{i} host1 sshd[1{i}]: Failed password for invalid user root from 8.8.4.4 port 55{i} ssh2'
            for i in range(5)
        ]
        logs.append(
            'Jun 11 10:01:00 host1 sshd[99]: Accepted password for root from 8.8.4.4 port 5599 ssh2'
        )
        self.assertTrue(
            any(a.rule_id == 'DET-SSH-002' for a in run_detections(parse_events(logs)))
        )

    def test_powershell_detection(self):
        e = parse_event(
            'WinEvent Time=2026-06-11T12:06:00Z Host=win01 EventID=4104 User=adam Process=powershell.exe CommandLine="powershell -NoP -enc AAA"'
        )
        self.assertTrue(any(a.rule_id == 'DET-PS-001' for a in run_detections([e])))

    def test_port_scan_detection(self):
        logs = [
            f'2026-06-11T14:00:0{i}Z fw01 FW action=DENY src=10.0.0.5 dst=10.0.1.{i} dpt=445 proto=TCP msg="scan"'
            for i in range(9)
        ]
        self.assertTrue(
            any(a.rule_id == 'DET-NET-001' for a in run_detections(parse_events(logs)))
        )
...
class SocLogicTests(unittest.TestCase):
    def test_correlation_metrics_anomaly(self):
        ...
        al = run_detections(ev)
        inc = correlate(al)
        m = calculate_metrics(ev, al, inc)
        an = detect_anomalies(ev)
        self.assertEqual(m['total_events'], 6)
        self.assertGreaterEqual(len(inc), 1)
        self.assertGreaterEqual(len(an), 1)
...
class ApiTests(unittest.TestCase):
    ...
    def test_ingest_and_triage(self):
        ...
        r = c.post(
            ...
            json={
                'logs': [
                    'Jun 11 12:00:00 host1 sshd[1]: Accepted password for adam from 10.0.0.9 port 22 ssh2'
                ]
            },
        )
        ...
        active_alerts = c.get('/api/alerts', headers=AUTH).json()
        ...
        self.assertEqual(t.json()['status'], 'recorded')
