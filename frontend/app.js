...
async function api(path){const r=await fetch(API+path,{cache:'no-store'}); if(!r.ok) throw new Error(`${path} returned ${r.status}`); return r.json();}
async function load(){
  try{
    ...
    state=fallback; $('#backend-status').textContent='Backend offline · start FastAPI on http://localhost:8000'; $('#backend-status').classList.add('offline'); $('#soc-source').textContent='Unavailable'; $('#soc-mode').textContent='Offline warning';
  }
  render();
}
function render(){
