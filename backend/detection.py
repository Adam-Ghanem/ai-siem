...
def _matches_static_conditions(rule: dict, event: Event) -> bool:
    ...
    for field, candidates in rule.get("contains", {}).items():
        ...
        if candidates and not any(str(candidate).lower() in value for candidate in candidates):
            return False
...
def _entity(rule: dict, event: Event) -> tuple:
    return rule["rule_id"], event.src_ip, event.user, event.asset


def _suppressed(cache: dict, rule: dict, event: Event) -> bool:
    entity = _entity(rule, event)
    previous = cache.get(entity)
    if previous and (event.timestamp - previous).total_seconds() < SUPPRESSION_MINUTES * 60:
        return True
    cache[entity] = event.timestamp
    return False


def _alert(rule: dict, events: list[Event]) -> Alert:
...
def run_detections(events: list[Event]) -> list[Alert]:
    ...
    failures: dict[tuple[str | None, str | None], list[Event]] = defaultdict(list)
    ...
    for index, event in enumerate(ordered_events):
        ...
        for rule in RULES:
            ...
            if rule["rule_id"] == "DET-SSH-002":
                previous_failures = [
                    failure
                    for (source_ip, user), grouped_failures in failures.items()
                    if source_ip == event.src_ip and (user == event.user or user is None)
                ...
                if (
                    len(previous_failures) >= rule["threshold"]
            ...
            window = [
                ...
                if _matches_static_conditions(rule, candidate)
                and _group_key(candidate, group_fields) == key
                and 0
                <= (event.timestamp - candidate.timestamp).total_seconds()
                <= rule.get("time_window_minutes", 1) * 60
            ]
            ...
            if (
                ...
                alerts.append(_alert(rule, window))
        ...
        if (
            ...
            is_new_external_source = event.src_ip not in user_sources and _is_external_ip(event.src_ip)
            under_limit = (
