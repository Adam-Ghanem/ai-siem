...
def load_events():
    if AI_SIEM_STORAGE=='sqlite':
        init_db(); stored=load_stored_events(limit=MAX_IN_MEMORY_EVENTS)
        if stored: return stored
        sample=_load_sample_events(); save_events(sample); return sample
...
async def security_middleware(request:Request, call_next):
    ...
    try: enforce_rate_limit(request); enforce_auth(request)
...
async def validation_error(request:Request, exc:RequestValidationError):
    audit_log(request,'validation','failed'); return JSONResponse(status_code=400,content={'detail':'Invalid request format','errors':exc.errors()})
@app.exception_handler(ValueError)
async def value_error(request:Request, exc:ValueError):
    audit_log(request,'validation','failed'); return JSONResponse(status_code=400,content={'detail':str(exc)})
@app.get('/api/health')
...
def get_metrics():
    ...
    m['parsing_failed_events']=unknown
    m['unknown_event_rate_pct']=round((unknown/max(len(EVENTS),1))*100,2)
    return m
...
async def ingest(request:Request):
    try: payload=await request.json()
        ...
        audit_log(request,'ingest','invalid_json'); raise HTTPException(status_code=400,detail='Invalid JSON body')
    items=_extract_items(payload); before_stats = parser_stats(); parsed=parse_events(items); EVENTS.extend(parsed)
    if AI_SIEM_STORAGE=='sqlite': save_events(parsed)
    after_stats = parser_stats(); audit_log(request,'ingest','success',f'count={len(parsed)}')
...
async def triage(request:Request):
    try: payload=await request.json()
        ...
        audit_log(request,'triage','invalid_json'); raise HTTPException(status_code=400,detail='Invalid JSON body')
